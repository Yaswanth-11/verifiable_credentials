...
import { sd_hash } from "./sd_hash.js"
...
export default class Holder {
  ...
  present = async ({ credential, disclosure, aud, nonce }) => {
    ...
    } = await Parse.expload(credential, { digester })
    const state = {
      hs_disclosures: [],
    ...
    if (sd_jwt_payload.cnf && (!aud || !nonce)) {
      throw new Error(
        "Credential requires confirmation but audience and nonce are missing."
      )
    }
    ...
    if (aud && nonce) {
      if (!sd_jwt_payload.cnf) {
        throw new Error(
          "Credential does not contain confirmation method, therefore audience and nonce are not supported."
        )
      }
      if (!signer) {
      ...
      const kbt = await signer.sign({
        protectedHeader: sortProtectedHeader({
          alg,
          kid,
          typ: KB_JWT_TYP_HEADER
        }),
        claimset: {
          iss,
          iat,
