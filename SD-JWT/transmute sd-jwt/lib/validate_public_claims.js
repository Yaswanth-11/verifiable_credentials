...
const acceptableAud = (expectedAud, receivedAud) => {
  return Array.isArray(receivedAud)
    ? receivedAud.includes(expectedAud)
    : receivedAud === expectedAud
}

export const validate_public_claims = (token_name, claimset, options) => {
  ...
  if (aud) {
    if (!reference_audience || !acceptableAud(reference_audience, aud)) {
      throw new Error(
        `${token_name} presented audience does not match reference value: ${reference_audience}`
      )
    }
  }
  ...
  if (nonce !== undefined) {
    if (reference_nonce !== nonce) {
      throw new Error(
        `${token_name} presented nonce does not match reference value: ${reference_nonce}`
      )
    }
  }
  ...
  if (iat) {
    ...
    if (isIssuedInFuture) {
      throw new Error(
        `${token_name} cannot be issued in the future...` +
          issuanceTime.fromNow()
      )
    }
  } else {
    if (debug) {
  ...
  if (nbf) {
    ...
    if (isActivationInFuture) {
      throw new Error(
        `${token_name} cannot be activated in the future...` +
          activationTime.fromNow()
      )
    }
  } else {
    if (debug) {
  ...
  if (exp) {
    ...
    if (isExpirationInPast) {
      throw new Error(
        `${token_name} cannot be expired in the past...` +
          expirationTime.fromNow()
      )
    }
  } else {
    if (debug) {
