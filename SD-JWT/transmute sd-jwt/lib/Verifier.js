...
import { validate_public_claims } from "./validate_public_claims.js"
import { validate_sd_hash } from "./validate_sd_hash.js"
...
export default class Verifier {
  ...
  verify = async ({ presentation, aud, nonce }) => {
    ...
    if (verifier) {
      ...
      if (!decodedHeader.kid) {
        throw new Error(
          "kid is required when resolver is used to obtain public keys"
        )
      }
    ...
    validate_public_claims(
      "Issuer-signed JWT",
      verifiedIssuanceToken.claimset,
      {
        debug,
    ...
    if (debug) {
      console.info(
        "Verified Issuer-signed JWT: ",
        JSON.stringify(verifiedIssuanceToken, null, 2)
      )
    }
    ...
    if (cnf) {
      if (!kbt) {
        throw new Error(
          "Verification of this credential requires proof of posession from the holder. Key binding token is expected based on claims, but was not found."
        )
      }
      try {
        let jkt
    ...
    const {
      disclosureMap,
      hashToEncodedDisclosureMap
    } = await Parse.expload(presentation, { digester: digester })
    const state = {
      _hash_to_disclosure: hashToEncodedDisclosureMap,
      _hash_to_decoded_disclosure: disclosureMap
    }
    const output = _unpack_disclosed_claims(
      verifiedIssuanceToken.claimset,
      state
    )
    return JSON.parse(
      JSON.stringify({
        protectedHeader: verifiedIssuanceToken.protectedHeader,
        claimset: output
      })
    )
  }
