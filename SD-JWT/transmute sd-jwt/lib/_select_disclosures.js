...
const _select_disclosures_list = (sd_jwt_claims, claims_to_disclose, state) => {
  ...
  for (const index in sd_jwt_claims) {
    ...
    if (itemHasDisclosure(element)) {
      ...
      const [salt, disclosure_value] = state._hash_to_decoded_disclosure[
        digest_to_check
      ]
      if (
        claims_to_disclose_element === false ||
        claims_to_disclose_element === undefined
      ) {
        continue
      }
      ...
      if (Array.isArray(disclosure_value)) {
        ...
        if (typeof claims_to_disclose_element !== "object") {
          throw new Error(
            "To disclose array elements nested in arrays, provide an array (can be empty)."
          )
        }
        ...
        if (typeof claims_to_disclose_element !== "object") {
          throw new Error(
            "To disclose object elements in arrays, provide an object (can be empty)."
          )
        }
        _select_disclosures(disclosure_value, claims_to_disclose_element, state)
      }
  ...
  for (const [key, value] of Object.entries(sd_jwt_claims)) {
    if (key === SD_DIGESTS_KEY) {
      for (const digest_to_check of value) {
        ...
        const [salt, key, value] = state._hash_to_decoded_disclosure[
          digest_to_check
        ]
        try {
