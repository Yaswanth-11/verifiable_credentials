...
export const validate_sd_hash = async (
  presentation_token,
  key_binding_sd_hash,
  debug = false
) => {
  ...
  const isPresentedTokenCommitedToInKeyBindingToken =
    presentation_digest === key_binding_sd_hash
  if (debug) {
    console.info("Key Binding Token sd_hash matches presentation token")
  }
  if (!isPresentedTokenCommitedToInKeyBindingToken) {
    throw new Error(
      "Key Binding Token sd_hash does not match presentation token"
    )
  }
}
