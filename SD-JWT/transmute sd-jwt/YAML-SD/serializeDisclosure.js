...
import { YAMLMap, YAMLSeq, Pair, parse,Scalar } from "yaml"

const serializeList = list => {
  return JSON.stringify(JSON.parse(JSON.stringify(list)))
    .replace(/"\:/g, '": ')
    .replace(/,/g, ", ")
}
...
const serializeMap = map => {
  ...
  return JSON.stringify({ _sd, ...rest })
    .replace(/"\:/g, '": ')
    .replace(/,/g, ", ")
}
...
export const serializeDisclosure = (salt, item) => {
  if (item instanceof Pair) {
    if (item.value instanceof YAMLSeq) {
      ...
      return `["${salt}", ${JSON.stringify(item.key.value).replace(
        /,/g,
        ", "
      )}, ${serializeScalar(item.value)}]`
    }
    ...
    return `["${salt}", ${JSON.stringify(
      JSON.parse(JSON.stringify(parse(item.value)))
    ).replace(/\:/g, ": ")}]`
  }
}
